...
class PlotCanvas(FigureCanvas):
    ...
    def __init__(self,data=None,parent=None, title='',width=5, height=4, dpi=100):
        self.df = data
        ...
        # self.p = QTimer()
        # self.p.setInterval(1000)
        # self.p.timeout.connect(self.plot)
        # self.p.start()
    ...
    def plot(self):
        ...
        #     temp = df[df['SN']==sn]
        #     # ax.plot(temp['DateTime'],temp['actVal'],label=sn)
        #     ax = self.figure.add_subplot(111)
        #     ax.plot(temp['actVal'],label=sn)
            # ax.grid(True)
            # ax.set_title('Ultraturb LED Verifikation')
            # ax.set_xlabel('DateTime')
            # ax.set_ylabel('Trübung FNU')
            # ax.legend()
            # ax.xaxis.set_major_formatter(mdates.DateFormatter('%d.%m.%Y - %H:%M:%S'))
            # ax.xaxis.set_tick_params(rotation=90)
            # ax.set_ylim(min(df['actVal'])*0.9,max(df['actVal'])*1.1)
            # # ax1.xaxis_date()
...
#        self.initUI()
    ...
    def initUI(self):
        ...
        m = PlotCanvas(data=self.df, width=5, height=4)
        m.move(800,0)
        
        self.show()
        # # probethread = threading.Thread(target=self.read_probes())
        # # probethread.start()
        self.timer = QTimer()
    ...
    #     self.draw()  
        ...
        # self.plot = QTimer()
        # self.plot.setInterval(1000)
        # self.plot.timeout.connect(self.update_plot)
        # self.plot.start()
